...
/* ── Hide Streamlit toolbar so it doesn't clip page content ── */
...
/* ── Remove top gap left behind by hidden toolbar ── */
.block-container { padding-top: 1.2rem !important; padding-bottom: 0; }

/* ── Sidebar: starts from very top of viewport ── */
...
/* ── Page headers — used by submodules ── */
...
[data-testid="stSidebar"] .stRadio label { color:#c8d8e8 !important;font-size:0.88rem;padding:0.25rem 0; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] { gap:0.1rem; }

/* ── Widgets ── */
div[data-testid="stMetric"] { background:#0d1829;border:1px solid #1a2a40;border-radius:10px;padding:0.8rem; }
.stButton button { background:#0a2444;border:1px solid #00d4ff;color:#00d4ff;font-family:'IBM Plex Mono';font-size:0.82rem;border-radius:8px;transition:background 0.15s; }
.stButton button:hover { background:#0d3060; }
...
def get_focal_points():
    # Try loading from contacts.py if available
    try:
...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        "🏠 Situation", "🗺️ Flood Map", "📊 Model Data",
        "🌧️ Rainfall", "📱 Send Alert",
        "📅 Event History", "🎯 Prediction vs Actual", "📄 Reports",
...
if page == "🏠 Situation":
    ...
    <div class="alert-banner {alert_cfg['cls']}">
        <div>
            ...
            <div style='margin-top:0.5rem;color:#8ba3bc;font-size:0.8rem;'>📍 Lower Shire Valley — Chikwawa & Nsanje Districts</div>
        </div>
    ...
    with col_left:
        ...
        for _, row in district_df.iterrows():
            icon = {'ACTIVE':'🔴','WATCH':'🟡','CLEAR':'🟢'}.get(row['status'],'⚪')
            st.markdown(f"""
    ...
    try:
        ...
        st.warning(f"flood_map.py error: {e}")
    ...
    st.markdown("""<div class="main-header"><h1>📊 Model Performance</h1><p>Random Forest + XGBoost Ensemble · Sentinel-1 SAR + CHIRPS</p><span class="header-badge">TRAINED: FREDDY 2023 · TESTED: IDAI 2019</span></div>""", unsafe_allow_html=True)
    ...
    findings = [
        ("🌧️","Rainfall is the #1 signal","How much rain fell during and before the event matters most. More rain = more flooding. Uses CHIRPS satellite rainfall data."),
        ("🛰️","Satellite radar change is #2","When the Sentinel-1 radar signal drops sharply, water has replaced dry land. This change is detected automatically."),
        ("🌊","Distance to Shire River is #3","Communities within 5km of the Shire River flood first. Geography is a strong predictor."),
    ...
    with st.expander("📈 Full technical details — for researchers"):
        shap_df = pd.DataFrame([
            ...
            {'Feature':'Distance to Shire River','Importance':1.41,'Type':'Terrain'},
        ...
        fig_shap = px.bar(shap_df,x='Importance',y='Feature',color='Type',
                          color_discrete_map={'SAR':'#00d4ff','Terrain':'#00cc66','Rainfall':'#ff8800'},
                          orientation='h',labels={'Importance':'SHAP feature importance','Feature':''})
        fig_shap.update_layout(paper_bgcolor='#080f1c',plot_bgcolor='#080f1c',font=dict(color='#c8d8e8'),
                               ...
                               legend=dict(bgcolor='#0d1829',bordercolor='#1a2a40'),
    ...
    st.markdown("""<div class="main-header"><h1>🌧️ Rainfall Monitor</h1><p>CHIRPS satellite rainfall · Updated daily at 06:00 UTC</p><span class="header-badge">LOWER SHIRE VALLEY</span></div>""", unsafe_allow_html=True)
    c1,c2,c3,c4 = st.columns(4)
    ...
    st.markdown("""<div class="main-header"><h1>📱 Send SMS Alert</h1><p>Africa's Talking API · Airtel Malawi & TNM</p><span class="header-badge">COMMUNITY FOCAL POINTS</span></div>""", unsafe_allow_html=True)
    ...
    try:
        ...
        import historical_comparison as _hc
        importlib.reload(_hc)
        # Call the dedicated prediction vs actual view
        _hc.show_prediction_vs_actual()
    except Exception as e:
